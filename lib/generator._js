...
function generateAspNet5DeploymentScript(scriptGenerator, _) {
  scriptGenerator.generateAspNet5DeploymentScript(_);
}
...
ScriptGenerator.prototype.generateAspNet5DeploymentScript = function (_) {
  ...
  if (this.scriptType != ScriptType.batch) {
    throw new Error('Only batch script files are supported for ASP.NET 5 Application');
  }

  log.info('Generating deployment script for ASP.NET 5 Application');

  var options = {
    kreProject: this.absoluteProjectPath,
    aspNet5Version: this.aspNet5Version,
    aspNet5Runtime: this.aspNet5Runtime,
    aspNet5Architecture: this.aspNet5Architecture
  };

  this.generateKDeploymentScript('deploy.batch.aspnet.5.template', options, _);
};
...
ScriptGenerator.prototype.generateKDeploymentScript = function (templateFileName, options, _) {
  argNotNull(templateFileName, 'templateFileName');
  ...
  ]).replace(/{PROJECT_JSON}/g, fixPathSeperatorToWindows(options.kreProject))
    .replace(/{ASPNET5_VERSION}/g, options.aspNet5Version)
    .replace(/{ASPNET5_RUNTIME}/g, options.aspNet5Runtime)
    .replace(/{ASPNET5_ARCHITECTURE}/g, options.aspNet5Architecture);

  this.writeDeploymentFiles(templateContent, _);
};

ScriptGenerator.prototype.generateDnxConsoleAppScript = function (templateFileName, options, _) {
  argNotNull(templateFileName, 'templateFileName');
  ...
  var templateContent = getTemplatesContent([
    ...
    templateFileName,
    'deploy.batch.postfix.template'
  ]).replace(/{WEBJOB_PROJECT_PATH}/g, fixPathSeperatorToWindows(options.dnxConsoleAppPath));
