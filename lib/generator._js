...
function ScriptGenerator(repositoryRoot, projectType, projectPath, solutionPath, sitePath, scriptType, scriptOutputPath, noDotDeployment, noSolution, logger, confirmFunc) {
  ...
  this.generators[ProjectType.console] = generateConsoleDeploymentScript;
}

function generateConsoleDeploymentScript(scriptGenerator, _) {
  scriptGenerator.generateConsoleDeploymentScript(_);
}
...
ScriptGenerator.prototype.generateConsoleDeploymentScript = function (_) {
  argNotNull(this.projectPath, 'projectPath');
  ...
  log.info('Generating deployment script for Console Worker');
  ...
  this.generateDotNetDeploymentScript('deploy.batch.console.template', options, _);
};
