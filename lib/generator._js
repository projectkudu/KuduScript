...
function ScriptGenerator(repositoryRoot, projectType, projectPath, solutionPath, sitePath, scriptType, scriptOutputPath, noDotDeployment, noSolution, logger, confirmFunc, useMSBuild) {
  ...
  if (!scriptType) {
    // If no script type is passed, use the default one (batch for Windows or bash for others)
    ...
    if (!isWindows && scriptType == ScriptType.batch) {
      throw new Error('Script type batch can only be specified on Windows');
    }
  }
...
function generateDnxConsoleAppDeploymentScript(scriptGenerator, _) {
  scriptGenerator.generateDnxConsoleAppDeploymentScript(_);
}
...
ScriptGenerator.prototype.generateFunctionAppDeploymentScript = function(_) {
    ...
    this.generateBasicDeploymentScript('functionApp.template', _);
};
...
ScriptGenerator.prototype.generateAspNetCoreDeploymentScript = function (_) {
  ...
  var nuget_35_renamed = "nuget.exe" // TODO rename the nuget.exe 3.5, default nuget.exe is 4.0 and above
  var nugetRestore = nuget_35_renamed + ' restore -packagesavemode nuspec';

  if (this.solutionPath && !(this.projectPath.endsWith(".csproj"))) {
    // using msbuild template if the project is created with preview2 and has a solution file
    if (fs.lstatSync(this.absoluteProjectPath).isDirectory()) {
        // old kudu args for preview2 project
        var files = fs.readdirSync(this.absoluteProjectPath);
        var webProjFiles = files.filter(function(file){
            return file.endsWith(".xproj");
        });
        if (webProjFiles.length != 1) {
            throw new Error('Expecting one xproj files but found ' + webProjFiles.length + ' under' + this.absoluteProjectPath);
        }
        this.projectPath = path.relative(this.repositoryRoot, path.join(this.absoluteProjectPath, webProjFiles[0]));
    }

    var msbuildArguments;
    var dotnetpublishArguments;

    if (this.scriptType == ScriptType.batch) {
      msbuildArguments = '"%DEPLOYMENT_SOURCE%\\' + this.solutionPath + '" /nologo /verbosity:m /p:AutoParameterizationWebConfigConnectionStrings=false;Configuration=Release;UseSharedCompilation=false %SCM_BUILD_ARGS%';
      dotnetpublishArguments = '"%DEPLOYMENT_SOURCE%\\' + this.projectPath + '" /nologo /verbosity:m /t:GatherAllFilesToPublish /p:AutoParameterizationWebConfigConnectionStrings=false;Configuration=Release;UseSharedCompilation=false;PublishOutputPathNoTrailingSlash="%DEPLOYMENT_TEMP%" %SCM_BUILD_ARGS%';
    } else {
      msbuildArguments = '"$DEPLOYMENT_SOURCE\\' + this.solutionPath + '" /nologo /verbosity:m /p:AutoParameterizationWebConfigConnectionStrings=false`;Configuration=Release`;UseSharedCompilation=false` $SCM_BUILD_ARGS';
      dotnetpublishArguments = '"$DEPLOYMENT_SOURCE\\' + this.projectPath + '" /nologo /verbosity:m /t:GatherAllFilesToPublish /p:AutoParameterizationWebConfigConnectionStrings=false`;Configuration=Release`;UseSharedCompilation=false`;PublishOutputPathNoTrailingSlash="$DEPLOYMENT_TEMP" $SCM_BUILD_ARGS';
    }
    ...
    this.generateAspNetCoreScript('aspnet.core.msbuild.template', options, _);
  } else {
...
ScriptGenerator.prototype.generateDnxConsoleAppDeploymentScript = function (_) {
  if (this.scriptType != ScriptType.batch) {
    throw new Error('Only batch script files are supported for DNX Console Application');
  }

  log.info('Generating deployment script for DNX Console Application');

  var options = {
    dnxConsoleAppPath: this.dnxConsoleAppPath
  };

  this.generateDnxConsoleAppScript('deploy.batch.dnx.consoleapp.template', options, _);
}
...
ScriptGenerator.prototype.generateAspNetCoreScript = function (templateFileName, options, _) {
  ...
  var templateContent = getTemplatesContent([
    ...
    'deploy.' + lowerCaseScriptType + '.' + templateFileName,
    'deploy.' + lowerCaseScriptType + '.postfix.template'
  ]).replace(/{PROJECT}/g, fixPathSeperatorToWindows(options.aspNetCoreProject))
    .replace(/{SolutionPath}/g, options.solutionPath)
    .replace(/{MSBuildArguments}/g, options.msbuildArguments)
    .replace(/{Restore}/g, fixedRestore)
