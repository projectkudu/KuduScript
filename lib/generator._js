...
var ProjectType = {
  ...
  dotNetCoreFunctionApp: 'DOT_NET_CORE_FUNCTIONAPP',
  dotNetConsole: 'DOT_NET_CONSOLE',
  dotNetCoreConsole: 'DOT_NET_CORE_CONSOLE',
...
function ScriptGenerator(repositoryRoot, projectType, projectPath, solutionPath, sitePath, scriptType, scriptOutputPath, noDotDeployment, noSolution, logger, confirmFunc, targetFramework, useMSBuild) {
  ...
  if (targetFramework && targetFramework != null && targetFramework != "")
  {
    this.targetFramework = "-f " + targetFramework;
  }
  else
  {
  ...
  this.generators[ProjectType.dotNetCoreFunctionApp] = generateDotNetCoreFunctionAppDeploymentScript;
  this.generators[ProjectType.dotNetConsole] = generateDotNetConsoleDeploymentScript;
  this.generators[ProjectType.dotNetCoreConsole] = generateDotNetCoreConsoleDeploymentScript;
...
function generateDotNetCoreConsoleDeploymentScript(scriptGenerator, _) {
  scriptGenerator.generateDotNetCoreConsoleDeploymentScript(_);
}
...
function generateDotNetCoreFunctionAppDeploymentScript(scriptGenerator, _) {
  scriptGenerator.generateDotNetCoreFunctionAppDeploymentScript(_);
}
...
ScriptGenerator.prototype.generateAspNetCoreDeploymentScript = function (_) {
  ...
  options.TargetFramework = this.targetFramework;

  this.generateAspNetCoreScript('aspnet.core.template', options, _);
};

ScriptGenerator.prototype.generateDotNetCoreConsoleDeploymentScript = function (_) {
  argNotNull(this.absoluteProjectPath, 'absoluteProjectPath');
  // no need to check scriptType since dotnet core is supported on both linux and windows
  ...
  this.generateDotNetCoreScript('console.netcore.template', options, _);
};

ScriptGenerator.prototype.generateDotNetCoreFunctionAppDeploymentScript = function (_) {
  argNotNull(this.absoluteProjectPath, 'absoluteProjectPath');
  // no need to check scriptType since dotnet core is supported on both linux and windows
  ...
  this.generateDotNetCoreScript('function.netcore.template', options, _);
};
...
ScriptGenerator.prototype.generateDotNetCoreScript = function (templateFileName, options, _) {
  argNotNull(templateFileName, 'templateFileName');
  ...
  var templateContent = getTemplatesContent([
    ...
    .replace(/{RestoreArguments}/g, options.RestoreArguments)
    .replace(/{TargetFramework}/g, options.TargetFramework)
