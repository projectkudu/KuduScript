...
function ScriptGenerator(repositoryRoot, projectType, projectPath, solutionPath, sitePath, scriptType, scriptOutputPath, noDotDeployment, noSolution, logger, confirmFunc) {
  ...
  this.generators[ProjectType.dorNetConsole] = generateDotNetConsoleDeploymentScript;
  this.generators[ProjectType.basicConsole] = generateBaicConsoleDeploymentScript;
}
...
function generateBaicConsoleDeploymentScript(scriptGenerator, _) {
  log.info('Generating deployment script for basic console application');
  scriptGenerator.generateBasicDeploymentScript('basicconsole.template', _);
}
...
ScriptGenerator.prototype.generateDotNetConsoleDeploymentScript = function (_) {
  ...
  var msbuildArguments = '"%DEPLOYMENT_SOURCE%\\' + this.projectPath + '" /nologo /verbosity:m /t:Build /p:Configuration=Release;OutputPath="%DEPLOYMENT_TEMP%\\bin"';
