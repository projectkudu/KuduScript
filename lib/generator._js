...
function ScriptGenerator(repositoryRoot, projectType, projectPath, solutionPath, sitePath, scriptType, scriptOutputPath, noDotDeployment, noSolution, msbuildPath, logger, confirmFunc) {
...
ScriptGenerator.prototype.generateDotNetDeploymentScript = function (templateFileName, options, _) {
  ...
  if (options.msbuildPath !== undefined) {
    templateContent = templateContent.replace(/{MSBUILD_PATH}/g, options.msbuildPath);
  } else {
    templateContent = templateContent.replace(/{MSBUILD_PATH}/g, "%MSBUILD_PATH%");
  }
