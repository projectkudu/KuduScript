...
ScriptGenerator.prototype.generateAspNetCoreDeploymentScript = function (_) {
  ...
  if (this.solutionPath){
    ...
    var relativeXprojPath = path.relative(this.repositoryRoot, path.join(this.absoluteProjectPath, webProjFiles[0]))
    ...
    if (this.scriptType == ScriptType.batch) {
      msbuildArguments = '"%DEPLOYMENT_SOURCE%\\' + this.solutionPath + '" /nologo /verbosity:m /p:AutoParameterizationWebConfigConnectionStrings=false;Configuration=Release;UseSharedCompilation=false';
      msbuildArguments += ' %SCM_BUILD_ARGS%';
    ...
    this.generateAspNetCoreScript('aspnet.core.template', {aspNetCoreProject: this.absoluteProjectPath}, _);
  }
};
...
ScriptGenerator.prototype.generateAspNetCoreScript = function (templateFileName, options, _) {
  ...
  var templateContent = getTemplatesContent([
    ...
    'deploy.' + lowerCaseScriptType + '.' + templateFileName,
    'deploy.' + lowerCaseScriptType + '.postfix.template'
  ]).replace(/{PROJECT_JSON}/g, fixPathSeperatorToWindows(options.aspNetCoreProject))
