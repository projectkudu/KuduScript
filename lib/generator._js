...
function ScriptGenerator(repositoryRoot, projectType, projectPath, solutionPath, sitePath, scriptType, scriptOutputPath, noDotDeployment, noSolution, logger, confirmFunc) {
  ...
  if (!scriptType) {
    ...
    if (projectType === ProjectType.wap || projectType === ProjectType.website || projectType === ProjectType.projectK) {
...
function generateProjectKDeploymentScript(scriptGenerator, _) {
  scriptGenerator.generateProjectKDeploymentScript(_);
}
...
ScriptGenerator.prototype.generateProjectKDeploymentScript = function (_) {
  argNotNull(this.projectPath, 'projectPath');

  if (this.scriptType != ScriptType.batch) {
    throw new Error('Only batch script files are supported for Project K Application');
  }

  log.info('Generating deployment script for ProjectK Application');

  var options = {
    kreProject: this.projectPath
  };

  this.generateKDeploymentScript('deploy.batch.aspnet.k.template', options, _);
}
