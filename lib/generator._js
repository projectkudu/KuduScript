...
function generateMSBuildConsoleDeploymentScript(scriptGenerator, _) {
  scriptGenerator.generateMSBuildConsoleDeploymentScript(_);
}
...
ScriptGenerator.prototype.generateFunctionAppDeploymentScript = function (_) {
  ...
  if (this.projectPath) {
    ...
    this.generateFunctionAppScript('functiondotnet.template', options, _);
  } else {
    if (this.scriptType == ScriptType.bash) {
...
ScriptGenerator.prototype.generateWapDeploymentScript = function (_) {
  ...
  this.generateMSBuildDeploymentScript('aspnet.wap.template', options, _);
};
...
ScriptGenerator.prototype.generateAspNetCoreDeploymentScript = function (_) {
  ...
  this.generateDotNetCoreScript('aspnet.core.template', options, _);

};

ScriptGenerator.prototype.generateMSBuildConsoleDeploymentScript = function (_) {
  argNotNull(this.projectPath, 'projectPath');

  if (this.scriptType != ScriptType.batch && this.scriptType != ScriptType.posh) {
    throw new Error('Only batch and posh script files are supported for .NET Web Application with old-style csproj');
  }
  ...
  this.generateMSBuildDeploymentScript('msbuildconsole.template', options, _);
};
...
ScriptGenerator.prototype.generateDotNetConsoleDeploymentScript = function (_) {
  ...
  if (this.scriptType == ScriptType.posh) {
    scmBuildArguments = '$env:SCM_BUILD_ARGS';
  } else {
    scmBuildArguments = '%SCM_BUILD_ARGS%';
  }

  this.generateDotNetCoreScript('dotnetconsole.template', options, _);
};
...
ScriptGenerator.prototype.generateWebSiteDeploymentScript = function (_) {
  if (this.solutionPath) {
    ...
    this.generateMSBuildDeploymentScript('aspnet.website.template', { msbuildArguments: msbuildArguments }, _);
  } else {
    // Basic website
...
ScriptGenerator.prototype.generateMSBuildDeploymentScript = function (templateFileName, options, _) {
  argNotNull(templateFileName, 'templateFileName');
...
ScriptGenerator.prototype.generateDotNetCoreScript = function (templateFileName, options, _) {
  ...
  var templateContent = getTemplatesContent([
    ...
    .replace(/{RestoreArguments}/g, options.RestoreArguments)
    .replace(/{scmBuildArguments}/g, options.scmBuildArguments);
