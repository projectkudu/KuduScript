...
ScriptGenerator.prototype.generateAspNetCoreDeploymentScript = function (_) {
  ...
  if (this.scriptType != ScriptType.batch) {
    throw new Error('Only batch script files are supported for ASP.NET Core Application');
  }

  log.info('Generating deployment script for ASP.NET Core Application');

  var msbuildArguments = '"%DEPLOYMENT_SOURCE%\\' + this.solutionPath + '" /nologo /verbosity:m /p:deployOnBuild=True;AutoParameterizationWebConfigConnectionStrings=false;Configuration=Release;UseSharedCompilation=false;publishUrl="%DEPLOYMENT_TEMP%"';
  msbuildArguments += ' %SCM_BUILD_ARGS%';
  ...
  if (this.solutionPath) {
    this.generateAspNetCoreScript('deploy.batch.aspnet.core.msbuild.template', options, _);
  } else {
    this.generateAspNetCoreScript('deploy.batch.aspnet.core.template', options, _);
  }
};
