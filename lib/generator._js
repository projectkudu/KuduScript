...
ScriptGenerator.prototype.generateAspNetCoreDeploymentScript = function (_) {
  ...
  // find .xproj file, should be under projectPath
  ...
  if(webProjFiles.length != 1){
    throw new Error('Expecting one xproj files but found ' + webProjFiles.length + ' under' + this.projectPath);
  }
  var relativeXprojPath = path.relative(this.repositoryRoot, path.join(this.projectPath, webProjFiles[0]))
  ...
  var options = {
    aspNetCoreProject: this.absoluteProjectPath,
    msbuildArguments: msbuildArguments,
