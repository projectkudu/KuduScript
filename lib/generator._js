...
ScriptGenerator.prototype.generateAspNetCoreDeploymentScript = function (_) {
  ...
  var options = {RestoreArguments:((this.solutionPath) ? this.solutionPath:this.projectPath)}
  if (this.projectPath.endsWith(".xproj")){
      // template msbuild14
      // XXX xproj almost always comes with sln
      options.MSBuildArguments = options.RestoreArguments;
      options.MSPublishArguments = this.projectPath;
      this.generateAspNetCoreScript('aspnet.core.msbuild14.template', options, _);
  }else{
      // "project.json" or "csproj", we all use dotnet cli
      options.DotnetpublishArguments = this.projectPath;
      this.generateAspNetCoreScript('aspnet.core.template', options, _);
  }
  // TODO replace above code with the following
  // if (this.projectPath.endsWith("project.json")){
  //     // template dotnet
  //     // XXX preview2- projects created with dotnet cli/ yeoman
  //     // TODO dotnet restore should run without argument since dependencies in "project.json" works different than "xproj/csproj"
  //     options.DotnetpublishArguments = this.projectPath;
  //     this.generateAspNetCoreScript('aspnet.core.template', options, _);
  // } else if (this.projectPath.endsWith(".xproj")){
  //     // template msbuild14
  //     // XXX xproj almost always comes with sln
  //     options.MSBuildArguments = options.RestoreArguments;
  //     options.MSPublishArguments = this.projectPath;
  ...
  // } else {
  //     // template msbuild15
  //     // FIXME this will not work on linux
  //     // XXX msbuild15 cannot deal with xproj/project.json only csproj
  //     options.MSBuildArguments = this.projectPath
  //     this.generateAspNetCoreScript('aspnet.core.msbuild15.template', options, _);
  // }
};
